...
const INITIAL_CARDS = [
  { id: 'card1', name: 'RedotPay Primary', provider: 'RedotPay', currency: 'USD', initialBalance: 0 },
  { id: 'card2', name: 'City Bank Dual Currency', provider: 'City Bank', currency: 'USD', initialBalance: 0 },
];
...
const INITIAL_TRANSACTIONS = [
  ...
  // USD Purchases
  { id: 't3', date: '2026-08-02', type: 'USD_PURCHASE', amountBDT: 12500, amountUSD: 100, rate: 125, method: 'Bank Transfer', notes: 'Binance P2P' },
  { id: 't4', date: '2026-08-08', type: 'USD_PURCHASE', amountBDT: 6300, amountUSD: 50, rate: 126, method: 'bKash', notes: 'Local Seller' },
...
// --- UTILITIES ---
const formatBDT = (amount) => `৳${(amount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`;
...
export default function AdLedgerApp() {
  ...
  // Modal State
  const [activeModal, setActiveModal] = useState(null); // 'payment', 'usd', 'spend', null
  ...
  const metrics = useMemo(() => {
    ...
    let cardBalances = {};
    cards.forEach(c => cardBalances[c.id] = c.initialBalance);
    ...
    const avgUSDRate = totalUSDPurchased > 0 ? (totalBDTSpentOnUSD / totalUSDPurchased) : 125; // fallback rate
    
    // Estimate cost of ads in BDT based on average purchase rate
    ...
    // Global USD Wallet Balance (Purchased - Loaded to cards)
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'cards': return <CardsView cards={cards} metrics={metrics} transactions={transactions} />;
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transition-transform transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative md:flex flex-col`}>
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          <div className="mt-4 p-3 bg-slate-800 rounded-lg flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center text-white font-medium">ME</div>
            <div>
              <p className="text-sm font-medium text-white">Mehedi</p>
    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
          <button className="px-3 py-1 text-sm font-medium rounded bg-slate-100 text-slate-800">30 Days</button>
          <button className="px-3 py-1 text-sm font-medium rounded text-slate-500 hover:text-slate-800">This Month</button>
          <button className="px-3 py-1 text-sm font-medium rounded text-slate-500 hover:text-slate-800">All Time</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <MetricCard 
          ...
          icon={<ArrowDownRight size={20} className="text-green-600" />}
          bgColor="bg-green-50"
        />
        <MetricCard 
          title="Net Profit" 
          value={formatBDT(metrics.netProfitBDT)} 
        ...
        <MetricCard 
          ...
          value={formatUSD(metrics.totalAdSpendUSD)} 
          subtitle={`+ Tax ${formatUSD(metrics.totalTaxUSD)}`}
          icon={<Activity size={20} className="text-purple-600" />}
          bgColor="bg-purple-50"
        />
        <MetricCard 
          title="Current USD Balance" 
          value={formatUSD(metrics.globalUSDBalance)} 
          subtitle="Ready to load to cards"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            <StatRow label="Avg USD Exchange Rate" value={`৳${metrics.avgUSDRate.toFixed(2)}`} />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {recentTx.length === 0 && (
                <tr><td colSpan="5" className="px-5 py-8 text-center text-slate-500">No recent transactions</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
              <tr>
                ...
                <th className="px-5 py-4 text-right">Monthly Budget</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {clientStats.map(c => (
                <tr key={c.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4 text-right text-slate-600">{formatBDT(c.budget)}</td>
    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="p-2 border border-slate-300 rounded-lg bg-white text-slate-600 hover:bg-slate-50"><Download size={18} /></button>
        </div>
      </div>
              ...
              {filtered.map(tx => {
                ...
                <tr key={tx.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-3">
                    ...
                    {tx.type === 'USD_PURCHASE' && <div className="font-medium text-slate-800">Rate: ৳{tx.rate}</div>}
                  </td>
...
function CardsView({ cards, metrics, transactions }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl p-6 text-white shadow-md flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="mt-6 pt-4 border-t border-slate-700">
            <p className="text-sm">Avg. Purchase Rate: <span className="font-bold text-white">৳{metrics.avgUSDRate.toFixed(2)}</span></p>
          </div>
        </div>
          ...
          <div key={card.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="bg-blue-50 p-2 rounded-lg text-blue-600"><CreditCard size={24} /></div>
              </div>
            ...
            <div className="mt-6 flex gap-2">
              <button className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 py-2 rounded-lg text-sm font-medium transition-colors">Load Money</button>
              <button className="flex-1 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 py-2 rounded-lg text-sm font-medium transition-colors">Details</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
            <tr>
              ...
              <th className="px-5 py-3 text-right">Exchange Rate</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {usdPurchases.map(tx => (
              <tr key={tx.id} className="hover:bg-slate-50">
                ...
                <td className="px-5 py-3 text-right font-medium text-slate-800">৳{tx.rate}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function TransactionTypeBadge({ type }) {
  ...
  return (
    <span className={`px-2.5 py-1 rounded text-xs font-semibold border ${styles[type] || 'bg-slate-100 text-slate-600'}`}>
      {labels[type] || type}
    </span>
  );
}
...
function TransactionForm({ type, clients, cards, onSubmit, onCancel }) {
  ...
  const handleChange = (e) => {
    ...
    // Auto calculate rate for USD Purchase
    if (type === 'USD_PURCHASE') {
      if (name === 'amountBDT' && newFormData.amountUSD) {
        newFormData.rate = (parseFloat(value) / parseFloat(newFormData.amountUSD)).toFixed(2);
      } else if (name === 'amountUSD' && newFormData.amountBDT) {
        newFormData.rate = (parseFloat(newFormData.amountBDT) / parseFloat(value)).toFixed(2);
      }
    }
      ...
      {type === 'USD_PURCHASE' && (
        <>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>BDT Paid</label>
            ...
            <div>
              ...
              <input type="number" name="amountUSD" value={formData.amountUSD} onChange={handleChange} required placeholder="$0.00" className={inputClass} />
            </div>
          </div>
          <div>
            <label className={labelClass}>Exchange Rate (Calculated)</label>
            <input type="text" name="rate" value={formData.rate} readOnly className={`${inputClass} bg-slate-50 text-slate-500 cursor-not-allowed`} placeholder="Auto calculated" />
          </div>
        </>
      )}
