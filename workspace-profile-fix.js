...
function makeField(label, key, value, placeholder = '', type = 'text', onChange) {
  ...
  if (input.tagName === 'TEXTAREA') { input.rows = 3; input.value = value || ''; input.placeholder = placeholder; input.className = 'adlytic-settings-control adlytic-description-control'; }
  else { input.type = type; input.value = value || ''; input.placeholder = placeholder; input.className = 'adlytic-settings-control'; }
